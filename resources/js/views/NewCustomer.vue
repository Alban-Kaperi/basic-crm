<script setup>
import { useCustomersStore } from '../stores/customers'
import {storeToRefs} from "pinia";
import AddButton from "../components/AddButton.vue";
const store = useCustomersStore()

const {createCustomer} = store
const { newCustomer } = storeToRefs(store)

</script>
<template>
    <div class="relative flex flex-col items-center justify-center w-[400px] bg-gray-50 border rounded-md p-4">

        <div @click="$router.back()" class="absolute top-2 right-2 ">
                <span class="flex items-center justify-center w-7 h-7 bg-gray-100 rounded-full text-gray-900 hover:bg-gray-200 cursor-pointer">
                    <svg xmlns="http://www.w3.org/2000/svg"
                         viewBox="0 0 20 20" fill="currentColor"
                         class="w-6 h-6">
                        <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                    </svg>
                </span>
        </div>


        <div class="inline-block px-6 py-3 bg-gray-200 rounded-t-md">
            <h3 class="h3-kj">Create new Customer</h3>
        </div>

        <div class="flex flex-col w-[300px] justify-around pt-4 gap-y-2">

                <div>
                    <label class="label-kj" for="name">Name</label>
                    <input type="text" id="name" class="input-kj" v-model="newCustomer.name">
                </div>
                <div>
                    <label class="label-kj" for="address">Address:</label>
                    <textarea v-model="newCustomer.address" id="address" class="input-kj" rows="1"></textarea>
                </div>

                <div>
                    <label class="label-kj" for="postal_code">Postal code:</label>
                    <input v-model="newCustomer.postal_code" type="text" id="postal_code" class="input-kj">
                </div>
                <div>
                    <label class="label-kj" for="place">Place:</label>
                    <input v-model="newCustomer.place" type="text" id="place" class="input-kj">
                </div>

                <div>
                    <label class="label-kj" for="telephone">Telephone:</label>
                    <input v-model="newCustomer.telephone" type="text" id="telephone" class="input-kj">
                </div>
                <div>
                    <label class="label-kj" for="email">Email:</label>
                    <input v-model="newCustomer.email" type="email" id="email" class="input-kj">
                </div>

        </div>

        <div class="pt-4 flex justify-center">
            <AddButton @click="createCustomer" button-name="Create Customer" />
        </div>

    </div>
</template>
